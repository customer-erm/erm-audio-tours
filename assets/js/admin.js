...
(function($) {
    ...
    var ERMATAdmin = {
        ...
        bindEvents: function() {
            ...
            // Pick element button (main step)
            $(document).on('click', '.erm-at-pick-element', function(e) {
                e.preventDefault();
                self.openElementPicker($(this), 'main');
            });

            // Pick element button (sub-highlight)
            $(document).on('click', '.erm-at-pick-element-sh', function(e) {
                e.preventDefault();
                self.openElementPicker($(this), 'sub');
            });
        ...
        openElementPicker: function(button, type) {
            ...
            if (type === 'sub') {
                // For sub-highlights, store the sub-highlight element
                var subHighlight = button.closest('.erm-at-sub-highlight');
                var subIndex = subHighlight.data('index');
        ...
        receiveSelector: function(data) {
            console.log('receiveSelector called', data);
            console.log('ermAtPickerStep', window.ermAtPickerStep);

            if (!window.ermAtPickerStep) {
                console.warn('No ermAtPickerStep found');
                return;
            }

            var pickerInfo = window.ermAtPickerStep;
            var stepItem = pickerInfo.item;

            if (!stepItem) {
                // Fallback: try to find by step ID if provided
                if (data.stepId) {
            ...
            if (!stepItem) {
                console.warn('Could not find step item');
                return;
            }

            // IMPORTANT: Only update sub-highlight if explicitly set
            if (pickerInfo.type === 'sub') {
                console.log('Updating sub-highlight only');
                // Update sub-highlight selector ONLY
                var subHighlight = pickerInfo.subHighlight;
                if (subHighlight && subHighlight.length) {
                    subHighlight.find('.erm-at-sh-selector').val(data.selector);
                    subHighlight.find('.erm-at-sh-selector-name').val(data.selectorName);
                    subHighlight.find('.erm-at-sh-name').text(data.selectorName || data.selector);

                    // Highlight the updated sub-highlight briefly
                    subHighlight.css('background', '#e0f7fa');
                    setTimeout(function() {
                        subHighlight.css('background', '');
                    }, 1500);
                } else {
                    console.warn('Sub-highlight element not found');
                }
                // DO NOT update main step - return early
                return;
            }

            // Only reach here if type is 'main' or undefined
            console.log('Updating main step only');
            // Update main step selector ONLY
