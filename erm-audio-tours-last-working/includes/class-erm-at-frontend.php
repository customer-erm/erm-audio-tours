...
class ERM_AT_Frontend {
    ...
    private function is_builder_active() {
        // Oxygen Builder
        if (isset($_GET['ct_builder']) || isset($_GET['oxygen_iframe']) || isset($_GET['oxy_user_library'])) {
            return true;
        }
        
        // Oxygen AJAX/render requests
        if (isset($_GET['action']) && strpos($_GET['action'], 'oxy_') === 0) {
            return true;
        }
